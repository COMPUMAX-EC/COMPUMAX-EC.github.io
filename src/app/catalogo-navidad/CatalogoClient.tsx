...
export default function CatalogoClient({ products }: { products: CatalogProduct[] }) {
  ...
  return (
    <main className="min-h-screen bg-[#062f2a] text-[#f9f1c7] selection:bg-[#f7c34d] selection:text-[#062f2a]">
      ...
      {isCartOpen && <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/65 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Carrito de compras"><div className="w-full max-w-md rounded-t-2xl border border-[#438bc0] bg-[#073b34] p-4 sm:rounded-2xl"><div className="flex items-center justify-between"><h2 className="text-xl font-bold text-[#f7c34d]">Tu carrito</h2><button onClick={() => setIsCartOpen(false)} aria-label="Cerrar carrito" className="rounded-full border border-[#3d83b8] px-3 py-1 text-lg">×</button></div>{cartItems.length ? <div className="mt-4 space-y-3">{cartItems.map(({ product, quantity }) => <div key={product.name} className="flex items-center justify-between gap-3 rounded-lg border border-[#438bc0] bg-[#0a466b] p-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{product.name}</p><p className="text-xs text-[#a9c8b2]">{quantity} × ${product.price}</p></div><strong className="text-sm text-[#f7c34d]">${(Number(product.price.replace(',', '.')) * quantity).toFixed(2)}</strong></div>)}</div> : <p className="mt-5 rounded-lg border border-dashed border-[#438bc0] p-4 text-center text-sm text-[#c9dcc5]">Aún no has añadido productos.</p>}<div className="mt-5 flex items-center justify-between border-t border-[#438bc0] pt-4"><span>Total</span><strong className="text-lg text-[#f7c34d]">${cartTotal.toFixed(2)}</strong></div><button onClick={requestQuote} className="mt-4 w-full rounded-lg bg-[#25D366] py-3 text-sm font-bold text-[#062f2a]">Cotizar por WhatsApp y pedir número de cuenta</button></div></div>}
      {selected && <div className="fixed inset-0 z-20 flex items-end justify-center bg-black/65 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label={`Detalle de ${selected.name}`}><div className="w-full max-w-md overflow-hidden rounded-t-2xl border border-[#438bc0] bg-[#073b34] sm:rounded-2xl"><div className="relative h-64 bg-[#0a466b]"><img src={selected.image} alt={`Detalle de ${selected.name}`} className="h-full w-full object-cover object-top" /><button onClick={() => setSelected(null)} aria-label="Cerrar detalle" className="absolute right-3 top-3 rounded-full bg-[#073b34]/85 px-3 py-1 text-lg">×</button></div><div className="p-4"><p className="text-[10px] uppercase tracking-widest text-[#9bc1ad]">{selected.category}</p><h2 className="mt-1 text-2xl font-bold text-[#f7c34d]">{selected.name}</h2><p className="mt-1 text-sm text-[#d4e0c5]">{selected.description || 'Diseño navideño listo para personalizar y sumar a tu decoración.'}</p><div className="mt-4 flex items-center justify-between"><span className="text-sm text-[#a9c8b2]">Cantidad</span><div className="flex items-center rounded-lg border border-[#438bc0]"><button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 py-2">−</button><span className="border-x border-[#438bc0] px-4 py-2 text-sm">{quantity}</span><button onClick={() => setQuantity(quantity + 1)} className="px-3 py-2">+</button></div></div><button onClick={() => addToCart(selected)} className="mt-4 w-full rounded-lg bg-[#f7c34d] py-3 text-sm font-bold text-[#073b34]">Añadir al carrito · ${selected.price}</button></div></div></div>}
    </main>
  )
}
